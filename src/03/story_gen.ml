...
and action = Eat | GoToRestaurant | EatHuman of character

and location = Apartment | Restaurant;;

let compatible_actions_for_character character context = match character with
...
let apply event context =
    let rec aux = function
        | [] -> assert false
        | character :: cs -> 
            if character = character_of_event event then match event with
...
let rec is_one_of state states = match states with
    | [] -> false
...
let something_happens context = 
    let what_can_happen = compatible_actions context @ possible_changes context
    in
...
let make_story initial_context = 
    let perturbation, context =
        something_happens initial_context in
    let conclusion, adventure =
        end_story [] context in
    {
...
let describe_character character =
    character.name ^ " was "
...
let tell_context context =
    let rec aux = function
        | [] -> ""
        | c :: cs -> describe_character c ^ aux cs
        in
    aux context.characters
;;
...
let tell_event = function
    | Action (character, action) -> character.name ^ " " ^ tell_action action ^ ". "
    | Change (character, state) -> character.name ^ " was made "
