...
let is_valid_answer ((grand_father_age: int), (grand_son_age: int)): bool =
  let quad_gs_age = (grand_son_age * 4) in
  let reved = exchange quad_gs_age in
  let triple = reved * 3 in
  let result = triple = grand_father_age in result;;
...
let rec find (answer: grand_age): grand_age =
  if is_valid_answer answer
    ...
    let min_grand_son_age, max_grand_father_age = answer in
